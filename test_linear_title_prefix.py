...
class BuildIssueTitleUpdateTest(unittest.TestCase):
    def test_adds_prefix_for_current_cursor_status_change_trigger_context(self):
        event = {
            "triggerContext": {
                ...
                "id": "POI-4538",
                "title": (
                    "[Container Logic Mass balance] In mass balance unable to see if a "
                    "batch comes from another site -> confusing"
                ),
            }
        }
        ...
        self.assertEqual(
            build_issue_title_update(event),
            {
                ...
                "issueId": "POI-4538",
                "title": (
                    "Cursor researching: [Container Logic Mass balance] In mass balance "
                    "unable to see if a batch comes from another site -> confusing"
                ),
            },
        )
    ...
    def test_ignores_missing_issue_id_or_title(self):
        ...
        self.assertIsNone(
            build_issue_title_update(
                {"trigger": "status_changed", "newStatus": "to research", "id": "POI-4538"}
            )
        )
        self.assertIsNone(
            build_issue_title_update(
