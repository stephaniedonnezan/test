...
class BuildIssueTitleUpdateTest(unittest.TestCase):
    def test_prefixes_cursor_trigger_context_when_status_moves_to_research(self):
        ...
        self.assertEqual(
            build_issue_title_update(event),
            {
                ...
                "title": (
                    "Cursor researching: "
                    "Backend e2e to test scenarios is escapable by user"
                ),
            },
        )
    ...
    def test_accepts_status_change_trigger_variant(self):
        event = {
        ...
        self.assertEqual(
            build_issue_title_update(event),
            {
                "action": "update_issue_title",
                "issueId": "POI-2",
                "title": "Cursor researching: Variant trigger",
            },
        )
    ...
    def test_handles_nested_linear_update_with_updated_fields(self):
        event = {
            "action": "update",
            "type": "Issue",
            "updatedFields": ["state"],
    ...
    def test_handles_changes_object_target_status(self):
        event = {
            "action": "Issue Updated",
            "data": {"id": "POI-7", "title": "Changed status"},
            "changes": {
                "workflowState": {
                    "from": {"name": "Backlog"},
                    "to": {"name": "To Research"},
