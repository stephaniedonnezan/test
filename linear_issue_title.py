...
def add_research_prefix(title: str, prefix: str = RESEARCH_TITLE_PREFIX) -> str:
    """Add the prefix exactly once while preserving existing title text."""
...
def update_issue_title_for_status_change(
    ...
    *,
    ...
    """Prefix title when an issue transitions to the target status."""
...
def build_linear_title_update_payload(event: dict[str, Any]) -> dict[str, str] | None:
    """Build the minimal payload to update a Linear issue title."""
...
def main() -> int:
    """Read webhook payload from stdin and print update payload."""
