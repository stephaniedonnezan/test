...
RESEARCHING_MARKER = RESEARCH_PREFIX
...
def updated_title_for_status_change(payload: Mapping[str, Any]) -> str | None:
    """Return updated title for status payloads; otherwise None."""
    status = _normalize(payload.get("newStatus") or payload.get("status"))
