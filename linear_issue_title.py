...
def _extract_trigger_context(payload: dict[str, Any]) -> dict[str, Any]:
    """Support raw payloads and wrappers that include triggerContext."""
...
def maybe_update_issue_title(payload: dict[str, Any], marker: str = RESEARCHING_MARKER) -> str | None:
    """Return an updated title when issue status moves to 'to research'."""
    ...
    new_status = _normalize_token(
        str(context.get("newStatus") or context.get("statusType") or context.get("status", ""))
    )
