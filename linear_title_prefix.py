...
def _is_issue_webhook_type(webhook_type: str) -> bool:
    return _normalize_token(webhook_type) == ISSUE_WEBHOOK_TYPE
...
def derive_updated_title(payload: Mapping[str, Any]) -> str | None:
    ...
    if isinstance(webhook_type, str) and not _is_issue_webhook_type(webhook_type):
        return None
