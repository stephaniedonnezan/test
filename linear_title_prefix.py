...
The automation runner can pass either Cursor's compact trigger context or a
fuller Linear webhook payload. This module keeps the decision pure and testable:
it returns the update the runner should apply, or ``None`` when no update is
needed.
"""
...
_TITLE_KEYS = ("title", "issueTitle", "issue_title", "summary")
...
def _walk_mappings(root: Mapping[str, Any]) -> Iterable[Mapping[str, Any]]:
    """Yield likely trigger and issue mappings in priority order."""
...
def _is_status_change_event(contexts: Iterable[Mapping[str, Any]]) -> bool:
    ...
    for context in contexts:
        ...
        if _metadata_mentions_status_field(context):
            has_status_field_change = True

    return has_issue_update_event and has_status_field_change


def _metadata_mentions_status_field(context: Mapping[str, Any]) -> bool:
    for key in _UPDATE_FIELD_KEYS:
        value = context.get(key)
        if _status_from_field_updates(value) is not None or _field_collection_includes_status(value):
            return True

    for key in _CHANGE_KEYS:
        value = context.get(key)
        if _status_from_changes(value) is not None or _field_collection_includes_status(value):
            return True
...
def _field_collection_includes_status(value: Any) -> bool:
    ...
    if isinstance(value, Mapping):
        if any(_is_status_field_name(key) for key in value):
            return True
        return any(_is_status_field_name(value.get(key)) for key in ("field", "name", "key"))
...
def _find_direct_string(context: Mapping[str, Any], keys: Iterable[str]) -> str | None:
    normalized_context = {_normalize_key(key): value for key, value in context.items()}
    for key in keys:
        value = _string_value(normalized_context.get(_normalize_key(key)))
        if value:
            return value
    return None
...
def _is_status_field_name(value: Any) -> bool:
    ...
    if not text:
        return False
    return _normalize_key(text) in _STATUS_FIELD_NAMES
