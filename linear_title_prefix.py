...
def build_issue_title_update(event: Mapping[str, Any] | None) -> dict[str, str] | None:
    ...
    if not _is_status_change(payload):
        return None
    if _normalize_label(_first_present(payload, "newStatus", "new_status", "status", "state")) != TARGET_STATUS:
        return None

    issue_id = _as_text(_first_present(payload, "id", "issueId", "issue_id", "identifier"))
