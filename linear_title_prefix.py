"""Build Linear issue title updates for research handoffs.

Cursor automation and Linear webhook payloads can arrive in flat or nested
shapes. This module keeps the automation decision deterministic: when an issue
status changes to "to research", return the title update that adds the
...
def build_issue_title_update(event: Mapping[str, Any]) -> dict[str, str] | None:
    ...
    issue_id = _first_text(_issue_contexts(event), ("id", "issueId", "issue_id", "identifier"))
    title = _first_text(_issue_contexts(event), ("title", "name"))
    if issue_id is None or title is None:
        return None

    trimmed_title = title.strip()
    if _has_research_prefix(trimmed_title):
        updated_title = trimmed_title
    else:
        updated_title = f"{TITLE_PREFIX}: {trimmed_title}"
...
def _is_status_change_event(event: Mapping[str, Any]) -> bool:
    ...
    return bool(updated_fields & status_fields) and bool(event_kinds & issue_update_kinds)
...
def _status_name(event: Mapping[str, Any]) -> str | None:
    ...
    explicit_status = _first_status_text(status_contexts, explicit_status_keys)
