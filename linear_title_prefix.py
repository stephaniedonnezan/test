...
def build_issue_title_update(event: Mapping[str, Any] | None) -> dict[str, str] | None:
    ...
    helper keeps the output small so an integration layer can apply the returned
    update through the Linear API.
    """
    ...
    if not _is_target_status(payload):
        return None
...
def _is_target_status(payload: Mapping[str, Any]) -> bool:
    status = _first_present(payload, "newStatus", "new_status", "status")
    if _normalize_status(status) == TARGET_STATUS:
        return True

    state = payload.get("state")
    if isinstance(state, Mapping) and _normalize_status(state.get("name")) == TARGET_STATUS:
        return True
