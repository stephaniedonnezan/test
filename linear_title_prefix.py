...
def build_issue_title_update(event: Mapping[str, Any] | None) -> dict[str, str] | None:
    ...
    The automation payload can be flat or nested under ``triggerContext`` and
    Linear issue fields may be nested under ``data``/``issue``. This function
    keeps the output deliberately small so an integration layer can apply the
    returned update through the Linear API.
    """
    ...
    status = _normalize_status(_first_present(payload, "newStatus", "new_status", "status"))
    if status != TARGET_STATUS:
        state = payload.get("state")
        if (
            not isinstance(state, Mapping)
            or _normalize_status(state.get("name")) != TARGET_STATUS
        ):
            return None
...
def _is_status_changed(event: Mapping[str, Any], payload: Mapping[str, Any]) -> bool:
    trigger_value = _first_present(event, "trigger", "webhookType", "action", "type")
    payload_trigger_value = _first_present(
        payload, "trigger", "webhookType", "action", "type"
    )
    return (
        _normalize_event_name(trigger_value) == "statuschanged"
        or _normalize_event_name(payload_trigger_value) == "statuschanged"
    )
