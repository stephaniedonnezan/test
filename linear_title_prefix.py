...
def _merge_payload(event: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(trigger_context, Mapping):
        for wrapper in ("data", "issue"):
            nested = trigger_context.get(wrapper)
            if isinstance(nested, Mapping):
                payload.update(nested)
        payload.update(trigger_context)

    # Top-level fields should win over nested issue data in automation payloads.
...
def _updated_fields_include_status(payload: Mapping[str, Any]) -> bool:
    ...
    if isinstance(updated_from, Mapping):
        return any(_normalize(field) in status_fields for field in updated_from)
