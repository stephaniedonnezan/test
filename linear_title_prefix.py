...
def build_issue_title_update(event: Mapping[str, Any]) -> dict[str, str] | None:
    ...
    flat Cursor trigger contexts or nested Linear webhook payloads and apply the
    returned action through their Linear client.
    """

    if not isinstance(event, Mapping):
        return None

    context = _first_mapping(event, "triggerContext", "trigger_context")
    data = _first_mapping(event, "data")
...
def _extract_issue(
    ...
    candidates = (
        _first_mapping(data, "issue"),
        _first_mapping(event, "issue"),
        context,
        data,
        event,
    )
