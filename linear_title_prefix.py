...
def _is_status_change(payload: Mapping[str, Any]) -> bool:
    for key in ("trigger", "webhookType", "action", "type"):
        if _looks_like_status_change(payload.get(key)):
            return True
...
def _updated_fields_include_status(payload: Mapping[str, Any]) -> bool:
    ...
    return any(
        _compact_text(field) in {"status", "state", "workflowstate"}
...
def main() -> int:
    """Read a Linear event from stdin and print the title update action."""
