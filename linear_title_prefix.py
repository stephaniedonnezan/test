...
def _new_status(payload: Mapping[str, Any]) -> Optional[str]:
    for key in (
        "newStatus",
        "new_status",
        "statusName",
...
def _issue_id(payload: Mapping[str, Any]) -> Optional[str]:
    return _first_text(payload, ("id", "issueId", "issue_id", "identifier"))


def _first_text(payload: Mapping[str, Any], keys: Tuple[str, ...]) -> Optional[str]:
    for key in keys:
        value = payload.get(key)
