...
def _is_status_change_event(contexts: list[Mapping[str, Any]]) -> bool:
    ...
    if markers.intersection(_STATUS_CHANGE_MARKERS):
        return True
    if markers.intersection(_UPDATE_MARKERS) and _updated_fields_include_status(contexts):
        return True
...
def _updated_fields_include_status(contexts: list[Mapping[str, Any]]) -> bool:
    for context in contexts:
        for key in _UPDATED_FIELD_KEYS:
            ...
            for field_name in _field_names(fields):
                if _compact_text(field_name) in _STATUS_FIELD_NAMES:
                    return True
    return False


def _field_names(fields: Any) -> Iterable[Any]:
...
def _issue_id(
    ...
    for context in (title_context, *contexts):
...
def _normalize_status(value: str | None) -> str | None:
    ...
    return " ".join(re.split(r"[\s_-]+", spaced.casefold())).strip()
...
def main() -> int:
    ...
    json.dump(action or {}, sys.stdout)
    sys.stdout.write("\n")
    return 0
