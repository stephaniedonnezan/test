...
def build_issue_title_update(event: Mapping[str, Any] | None) -> dict[str, str] | None:
    ...
    Automation payloads may be flat, nested under ``triggerContext``, or shaped
    like Linear webhook payloads with issue fields under ``data`` or ``issue``.
    """
    ...
    if _current_status(payload) != TARGET_STATUS:
        return None
...
def _current_status(payload: Mapping[str, Any]) -> str:
    explicit_status = _normalize_status(_first_present(payload, "newStatus", "new_status", "status"))
    if explicit_status:
        return explicit_status

    state = payload.get("state")
    if isinstance(state, Mapping):
        return _normalize_status(state.get("name"))
