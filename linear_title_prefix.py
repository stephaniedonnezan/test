...
def _merge_context(event: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(issue, Mapping):
        context["issue"] = issue
    elif isinstance(event.get("data"), Mapping):
        data_issue = event["data"].get("issue")
        if isinstance(data_issue, Mapping):
            context["issue"] = data_issue
...
def _is_status_change(context: Mapping[str, Any]) -> bool:
    ...
    if any(name in {"status changed", "status change", "statuschanged"} for name in normalized_events):
        return True

    if any(name in {"issue updated", "updated issue", "update", "updated"} for name in normalized_events):
...
def _updated_fields(context: Mapping[str, Any]) -> set[str]:
    raw_fields = _deep_get(context, "updatedFields") or _deep_get(context, "updated_fields")
