...
The automation runner can call ``build_issue_title_update`` with a Linear or
Cursor automation payload. When a status-change event moves an issue to
...
def _candidate_maps(value: Any) -> Iterable[Mapping[str, Any]]:
    if isinstance(value, Mapping):
        yield value
        for key in _CANDIDATE_MAP_KEYS:
