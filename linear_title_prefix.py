...
CHANGE_KEYS = {"trigger", "action", "type", "webhooktype", "webhook_type"}
...
def build_issue_title_update(event: Mapping[str, Any] | None) -> dict[str, str] | None:
    ...
    The automation runner performs side effects from the returned action. This
    function only decides whether the status-change payload requires a title
    prefix and keeps duplicate prefixes out of subsequent updates.
    """
...
def _is_status_change(contexts: list[Mapping[str, Any]]) -> bool:
    ...
    for context in contexts:
        for key, value in context.items():
            if not isinstance(value, str) or _compact_key(key) not in CHANGE_KEYS:
                continue
