...
def _contexts(event: Mapping[str, Any]) -> list[Mapping[str, Any]]:
    ...
    for key in ("triggerContext", "data", "issue", "state", "workflowState", "workflowStatus", "status"):
        value = event.get(key)
...
def _first_fallback_status(contexts: Iterable[Mapping[str, Any]]) -> str | None:
    for context in contexts:
        for key in ("status", "state", "workflowState", "workflow_state", "workflowStatus", "workflow_status"):
            value = context.get(key)
            if isinstance(value, Mapping):
                name = _first_value(value, ("name", "title", "status"))
                if name is not None:
