...
def build_issue_title_update(event: Mapping[str, Any] | None) -> dict[str, str] | None:
    ...
    The automation trigger may pass Cursor metadata at the top level, under
    ``triggerContext``, or alongside a nested Linear ``issue`` object. This
    function accepts those shapes and returns a serializable update operation
    ...
    if _normalize_status(_first_value(payload, ("newStatus", "new_status", "status"))) != TARGET_STATUS:
        return None
...
def _is_status_changed(payload: Mapping[str, Any]) -> bool:
    ...
    return _normalize_status(trigger) == STATUS_CHANGED
