...
def _status_is_target(mappings: list[Mapping[str, Any]]) -> bool:
    explicit_statuses = list(_values_for_keys(mappings, _EXPLICIT_STATUS_KEYS))
    if explicit_statuses:
        return any(_normalize(_status_name(status)) == TARGET_STATUS for status in explicit_statuses)

    current_statuses = list(_values_for_keys(mappings, _CURRENT_STATUS_KEYS))
    if any(_normalize(_status_name(status)) == TARGET_STATUS for status in current_statuses):
        return True

    return any(_change_target_is_status(change, TARGET_STATUS) for change in _values_for_keys(mappings, ("changes",)))
...
def _change_target_is_status(change: Any, target_status: str) -> bool:
    if isinstance(change, Mapping):
        for key, value in change.items():
            if _normalize(key) in {"to", "new", "after"}:
                return _normalize(_status_name(value)) == target_status
            if _is_status_field(key) and _change_target_is_status(value, target_status):
                return True
    return False
