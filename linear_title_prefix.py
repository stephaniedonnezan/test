...
def build_issue_title_update(event: Mapping[str, Any] | None) -> dict[str, str] | None:
    ...
    nested Linear webhook payloads. It returns data instead of calling Linear so
    the surrounding automation can decide how to apply the side effect.
    """
...
def _first_fallback_status(contexts: Iterable[Mapping[str, Any]]) -> str | None:
    for context in contexts:
        for key in ("status", "state", "workflowStatus", "workflow_status", "workflowState", "workflow_state"):
...
def _status_name(value: Any) -> str | None:
    if isinstance(value, Mapping):
        nested = _first_value(value, ("name", "title", "label"))
        if nested is not None:
            return _clean_string(nested)
        return None
