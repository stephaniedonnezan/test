...
def build_issue_title_update(event: Mapping[str, Any] | None) -> dict[str, str] | None:
    ...
    The returned action is transport-agnostic so the caller can map it to the
    actual Linear mutation layer.
    """
...
def _is_status_change_event(payload: Mapping[str, Any]) -> bool:
    trigger_values = [
        value for key, value in payload.items() if key in _TRIGGER_KEYS and isinstance(value, str)
    ]
    ...
    return has_status_value and any(_looks_like_issue_update(value) for value in trigger_values)
...
def _extract_changed_fields(payload: Mapping[str, Any]) -> list[str]:
    ...
    if isinstance(updated_from, Mapping):
        field_values.extend(str(key) for key in updated_from)
...
def _extract_status(payload: Mapping[str, Any]) -> str | None:
    ...
    for key in ("state", "workflowState", "status"):
        ...
        if isinstance(value, Mapping):
            nested_name = _first_text(value.get("name"), value.get("title"))
