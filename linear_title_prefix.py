...
def _is_status_change(event: Mapping[str, Any], payload: Mapping[str, Any]) -> bool:
    ...
    if any(value in {"issue updated", "updated issue"} for value in normalized_triggers):
        ...
        if isinstance(updated_fields, str):
            ...
            field_names = []
        return any(_normalize(_as_text(field)) in {"status", "state", "workflow state"} for field in field_names)
