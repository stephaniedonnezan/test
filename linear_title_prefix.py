...
def derive_updated_title(event: Mapping[str, Any] | None) -> str | None:
    """Return just the updated title for callers that do not need action metadata."""

    update = build_issue_title_update(event)
    return update["title"] if update else None


def updated_title_for_status_change(event: Mapping[str, Any] | None) -> str | None:
    """Compatibility wrapper that returns the updated title or None."""

    return derive_updated_title(event)


def update_issue_title_for_status(title: str, new_status: str) -> str:
    """Return title with the research prefix when status is `to research`."""
