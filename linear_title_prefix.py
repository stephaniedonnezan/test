...
_UPDATE_EVENT_VALUES = {"update", "updated", "issue updated", "updated issue"}
...
def _candidate_containers(event: Mapping[str, Any]) -> Iterable[Mapping[str, Any]]:
...
def _updated_fields_include_status(payload: Mapping[str, Any]) -> bool:
    ...
    if isinstance(updated_fields, str):
        return _normalize(updated_fields) in _STATUS_FIELD_VALUES

    if isinstance(updated_fields, Mapping):
        return any(_normalize(key) in _STATUS_FIELD_VALUES for key in updated_fields)

    if isinstance(updated_fields, list | tuple | set):
        return any(_normalize(field) in _STATUS_FIELD_VALUES for field in updated_fields)

    updated_from = payload.get("updatedFrom")
    if isinstance(updated_from, Mapping):
        return any(_normalize(key) in _STATUS_FIELD_VALUES for key in updated_from)
