...
def build_issue_title_update(event: Mapping[str, Any] | None) -> dict[str, str] | None:
    ...
    Cursor automation triggers provide compact, flat ``triggerContext`` payloads.
    Linear webhooks are often nested under ``data`` or ``issue``. This accepts
    both shapes and returns a side-effect-free action for the caller to apply
    through its Linear client.
    """
