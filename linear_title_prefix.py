...
def build_issue_title_update(event: Mapping[str, Any] | None) -> dict[str, str] | None:
    ...
    The Cursor automation trigger provides a flat ``triggerContext`` payload,
    Cloud automation payloads can wrap it under ``automation_trigger_info``, and
    Linear webhooks can nest issue details under ``data`` or ``issue``. This
    function accepts those shapes and returns ``None`` when no title update is
    needed.
    """
...
def _candidate_issue_maps(event: Mapping[str, Any]) -> list[Mapping[str, Any]]:
    ...
    automation_trigger_info = _first_mapping_for_keys(
        event,
        ("automation_trigger_info", "automationTriggerInfo", "automationInfo"),
    )
    if automation_trigger_info is not None:
        add(
            _first_mapping_for_keys(
...
def _first_mapping_for_keys(
    ...
    for key in keys:
        value = _value_for_normalized_key(mapping, key)
        if isinstance(value, Mapping):
            return value
    return None
