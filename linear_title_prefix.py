...
def build_issue_title_update(event: Mapping[str, Any]) -> dict[str, str] | None:
    ...
    while Linear webhooks commonly nest issue details under ``data.issue``.
    This function accepts both shapes and returns a serializable action for
...
def _field_collection_includes_status(fields: Any) -> bool:
    ...
    for field in fields:
        if _field_name_is_status(field):
            return True
    return False
...
def _extract_new_status(event: Mapping[str, Any]) -> str | None:
    ...
    for context in _iter_contexts(event):
        value = _first_value(context, status_keys)
        status = _string_or_name(value)
        if status:
            return status

    for context in _iter_contexts(event):
        value = _first_value(context, fallback_keys)
        status = _string_or_name(value)
        if status:
            return status
