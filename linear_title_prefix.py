...
def _is_status_change(context: Mapping[str, Any]) -> bool:
    trigger = _first_text(context, ("trigger", "webhookType", "action", "type"))
    if trigger and _normalize_text(trigger) in {
        "status changed",
        "status changed",
        "statuschanged",
        "issue updated",
    }:
        if _normalize_text(trigger) != "issue updated":
            return True
...
def _contains_status_field(value: Any) -> bool:
    ...
    if isinstance(value, Mapping):
        return any(
            _normalize_text(str(key)) in {"status", "state"}
