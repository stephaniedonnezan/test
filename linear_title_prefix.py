...
def build_issue_title_update(event: Mapping[str, Any] | None) -> dict[str, str] | None:
    ...
    The automation runner passes a flat ``triggerContext`` payload, while Linear
    webhooks may wrap issue data under ``data``/``issue``. This function accepts
    both shapes and returns a small action object for the caller to execute.
    """
    ...
    if _has_prefix(title):
        prefixed_title = title.strip()
    else:
        prefixed_title = f"{TITLE_PREFIX}: {title.strip()}"
...
def _payload(event: Mapping[str, Any]) -> dict[str, Any]:
    ...
    # Nested issue data should not overwrite outer webhook metadata such as
    # action/type or explicit newStatus values.
