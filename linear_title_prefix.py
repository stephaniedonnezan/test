...
def _extract_status(mappings: list[Mapping[str, Any]]) -> str | None:
    ...
    for mapping in mappings:
        value = _status_from_changes(mapping.get("changes"))
        if value:
            return value
...
def _status_from_changes(changes: Any) -> str | None:
    ...
    for key, value in changes.items():
        if _normalize_status(str(key)) not in {"status", "state", "workflow state"}:
            continue

        if isinstance(value, str) and value.strip():
            return value
        if isinstance(value, Mapping):
            status = _first_string(value, ("to", "new", "newValue", "name"))
            if status:
                return status
