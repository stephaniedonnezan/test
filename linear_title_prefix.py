...
def _merge_payload(event: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for wrapper in _iter_payload_layers(event):
        payload.update(wrapper)

    return payload


def _iter_payload_layers(event: Mapping[str, Any]) -> Iterable[Mapping[str, Any]]:
    for key in ("data", "issue"):
        nested = event.get(key)
        if isinstance(nested, Mapping):
            yield from _iter_payload_layers(nested)
            yield nested

    trigger_context = event.get("triggerContext")
    if isinstance(trigger_context, Mapping):
        yield from _iter_payload_layers(trigger_context)
