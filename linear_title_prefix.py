...
The automation runner may pass Cursor's ``triggerContext`` payload or a native
Linear webhook payload. This module keeps the detection logic side-effect free:
callers receive the intended title update action, or ``None`` when the event
should be ignored.
"""
...
def _unwrap_payload(event: Mapping[str, Any]) -> Mapping[str, Any]:
    """Prefer Cursor automation trigger context, otherwise keep the original payload."""
...
def _merge_payloads(outer: Mapping[str, Any], inner: Mapping[str, Any]) -> dict[str, Any]:
    merged = dict(inner)
...
def _extract_status_from_mapping(payload: Mapping[str, Any]) -> str | None:
    for key in DIRECT_STATUS_KEYS:
        status = _status_value_to_string(payload.get(key))
...
def _status_from_change_container(value: Any) -> str | None:
    ...
    if isinstance(value, Sequence) and not isinstance(value, (str, bytes, bytearray)):
        for item in value:
            ...
            if (
                field_name is None
                or _normalize_field_name(field_name) not in STATUS_FIELD_NAMES
            ):
                continue
