...
def build_issue_title_update(event: Mapping[str, Any]) -> dict[str, str] | None:
    """Return a Linear title update for issues moved to To Research.

    Automation payloads may place issue details at the top level or under
    ``triggerContext``, ``data``, or ``issue``. The returned dictionary is a
    small action object that downstream automation can use to update the issue.
    """
    ...
    if _normalize_words(_status_value(contexts)) != TO_RESEARCH_STATUS:
        return None
    ...
    return {
        ...
        "issueId": issue_id,
        "title": f"{RESEARCHING_PREFIX}: {title}",
    }


def handle_issue_status_changed(event: Mapping[str, Any]) -> dict[str, str] | None:
    """Compatibility entrypoint for status-change automation handlers."""
...
def _is_status_change(contexts: Iterable[Mapping[str, Any]]) -> bool:
    for context in contexts:
        for key in ("trigger", "action", "type", "eventType", "webhookType"):
            if _normalize_words(context.get(key)) in {"status changed", "status change"}:
                return True

        if _is_issue_updated(context) and _updated_fields_include_status(context):
            return True

    return False


def _is_issue_updated(context: Mapping[str, Any]) -> bool:
    for key in ("trigger", "action", "type", "eventType", "webhookType"):
        if _normalize_words(context.get(key)) in {
            "issue updated",
            "updated issue",
            "issue update",
            "update issue",
...
def _status_value(contexts: Iterable[Mapping[str, Any]]) -> Any:
    ...
    for context in contexts:
        for key in ("state", "workflowState", "workflow_state"):
            ...
            if nested is not None and nested.get("name") is not None:
                return nested["name"]

    return _first_value(contexts, ("status",))
...
def _has_researching_prefix(title: str) -> bool:
    return title.strip().lower().startswith(RESEARCHING_PREFIX.lower())
