...
class MaybeUpdateIssueTitleTests(unittest.TestCase):
    ...
    def test_accepts_status_type_to_research(self) -> None:
        payload = {
            "triggerContext": {
                "trigger": "status_changed",
                "statusType": "to_research",
                "title": "Investigate OCR extraction mismatch",
            }
        }

        updated = maybe_update_issue_title(payload)

        self.assertEqual(updated, f"{RESEARCHING_MARKER} - Investigate OCR extraction mismatch")
