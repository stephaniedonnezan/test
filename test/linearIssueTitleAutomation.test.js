...
test("withResearchingPrefix adds prefix and stays idempotent", () => {
  assert.equal(
    withResearchingPrefix("Implement Certifhy chip in audit"),
    `${RESEARCHING_TITLE_PREFIX}: Implement Certifhy chip in audit`,
  );
...
test("buildIssueTitleUpdate returns update payload for matching event", () => {
  ...
  assert.deepEqual(buildIssueTitleUpdate(payload), {
    issueId: "POI-4500",
    previousTitle: "Implement Certifhy chip in audit",
    nextTitle: "Cursor researching: Implement Certifhy chip in audit",
  });
});
