...
test("withResearchingPrefix adds prefix and stays idempotent", () => {
  assert.equal(
    withResearchingPrefix("WP1: Audit Overview & Management"),
    `${RESEARCHING_TITLE_PREFIX}: WP1: Audit Overview & Management`,
  );
...
test("buildIssueTitleUpdate returns update payload for matching event", () => {
  ...
  assert.deepEqual(buildIssueTitleUpdate(payload), {
    ...
    nextTitle: "Cursor researching: WP1: Audit Overview & Management",
  });
});
...
test("buildIssueTitleUpdate returns null for unrelated statuses", () => {
  const payload = {
    triggerContext: {
      trigger: "status_changed",
      newStatus: "in progress",
